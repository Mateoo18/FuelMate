...
from .models import Gas_Stations, PostalCode
import os
...
def search_stations_by_postal_code(request):
    ...
    stations = Gas_Stations.objects.filter(address__icontains=postal_code)
...
def logged_in_view(request):
    ...
    if postal_code:
        stations = list(Gas_Stations.objects.filter(postal_code=postal_code))
        ...
        if len(stations) < 10:
            ...
            if lat is not None and lon is not None:
                additional_stations = []
                all_stations = Gas_Stations.objects.exclude(postal_code=postal_code)
    ...
    if latitude and longitude:
        ...
        all_stations = Gas_Stations.objects.all()
